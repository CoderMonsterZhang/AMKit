...
#import "MasCustomNavigationBar.h"
#import "sys/utsname.h"

#define kMasDefaultTitleSize 18
#define kMasDefaultTitleColor [UIColor blackColor]
#define kMasDefaultBackgroundColor [UIColor whiteColor]
#define kMasScreenWidth [UIScreen mainScreen].bounds.size.width

@implementation UIViewController (MasRoute)

- (void)mas_toLastViewController
{
...
+ (UIViewController*)mas_currentViewController {
    UIViewController* rootViewController = [UIApplication sharedApplication].delegate.window.rootViewController;
    return [self mas_currentViewControllerFrom:rootViewController];
}

+ (UIViewController*)mas_currentViewControllerFrom:(UIViewController*)viewController
{
    if ([viewController isKindOfClass:[UINavigationController class]]) {
        UINavigationController* navigationController = (UINavigationController *)viewController;
        return [self mas_currentViewControllerFrom:navigationController.viewControllers.lastObject];
    }
    else if([viewController isKindOfClass:[UITabBarController class]]) {
        UITabBarController* tabBarController = (UITabBarController *)viewController;
        return [self mas_currentViewControllerFrom:tabBarController.selectedViewController];
    }
    else if (viewController.presentedViewController != nil) {
        return [self mas_currentViewControllerFrom:viewController.presentedViewController];
    }
    else {
...
@interface MasCustomNavigationBar ()
...
@implementation MasCustomNavigationBar

+ (instancetype)CustomNavigationBar {
    MasCustomNavigationBar *navigationBar = [[self alloc] initWithFrame:CGRectMake(0, 0, kMasScreenWidth, [MasCustomNavigationBar navBarBottom])];
...
-(void)setupView {
    ...
    self.backgroundView.backgroundColor = kMasDefaultBackgroundColor;
}

// TODO:这边结合 MasCellView 会不会更好呢？
-(void)updateFrame {
    NSInteger top = ([MasCustomNavigationBar isIphoneX]) ? 44 : 20;
    NSInteger margin = 0;
    ...
    self.rightButton.frame = CGRectMake(kMasScreenWidth - buttonWidth - margin, top, buttonWidth, buttonHeight);
    self.titleLable.frame = CGRectMake((kMasScreenWidth - titleLabelWidth) / 2, top, titleLabelWidth, titleLabelHeight);
    self.bottomLine.frame = CGRectMake(0, (CGFloat)(self.bounds.size.height-0.5), kMasScreenWidth, 0.5);
}
...
-(void)clickBack {
    if (self.onClickLeftButton) {
        ...
        UIViewController *currentVC = [UIViewController mas_currentViewController];
        [currentVC mas_toLastViewController];
    }
}
-(void)clickRight {
...
- (void)mas_setLeftButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mas_setLeftButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)mas_setLeftButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self mas_setLeftButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)mas_setLeftButtonWithImage:(UIImage *)image {
    [self mas_setLeftButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)mas_setLeftButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mas_setLeftButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
...
- (void)mas_setRightButtonWithImage:(UIImage *)image title:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mas_setRightButtonWithNormal:image highlighted:image title:title titleColor:titleColor];
}
- (void)mas_setRightButtonWithNormal:(UIImage *)normal highlighted:(UIImage *)highlighted {
    [self mas_setRightButtonWithNormal:normal highlighted:highlighted title:nil titleColor:nil];
}
- (void)mas_setRightButtonWithImage:(UIImage *)image {
    [self mas_setRightButtonWithNormal:image highlighted:image title:nil titleColor:nil];
}
- (void)mas_setRightButtonWithTitle:(NSString *)title titleColor:(UIColor *)titleColor {
    [self mas_setRightButtonWithNormal:nil highlighted:nil title:title titleColor:titleColor];
}
...
-(UILabel *)titleLable {
    if (!_titleLable) {
        ...
        _titleLable.textColor = kMasDefaultTitleColor;
        _titleLable.font = [UIFont systemFontOfSize:kMasDefaultTitleSize];
